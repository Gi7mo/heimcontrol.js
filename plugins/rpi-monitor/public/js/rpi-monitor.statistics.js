  ...
  function FetchGraph() {
    ...
    for ( var iloop = 0; iloop < graph.length; iloop++) {
      if (  ( static==null ) || ( eval ( "static."+graph[iloop] ) ) ){
        try {
          FetchBinaryURLAsync('/api/rpi-monitor/get?mode=rrd&type=empty', UpdateHandler, iloop);
        }
        catch (err) {
        ...
        try {
          FetchBinaryURLAsync('/api/rpi-monitor/get?mode=rrd&type=' + graph[iloop], UpdateHandler, iloop);
        }
        catch (err) {
