...
require(['jquery'], function() {
	...
	function UpdateStatus () {
		$.getJSON('/api/rpi-monitor/get?mode=json&type=dynamic', function(data) {
	...
	function getData(name) {
		if (localStorage.getItem(name+'Version') == localStorage.getItem('version') ) {
			...
			return JSON.parse($.ajax('/api/rpi-monitor/get?mode=json&type=' + name, {
				dataType: 'json',
				async: false
			}).done(function(data) {
	...
	function ConstructPage() {
		...
		for ( var iloop=0; iloop < data[activePage].content.length; iloop++) {
			$(RowTemplate(iloop,"img/rpi-monitor/"+data[activePage].content[iloop].icon,data[activePage].content[iloop].name)).insertBefore("#insertionPoint");
